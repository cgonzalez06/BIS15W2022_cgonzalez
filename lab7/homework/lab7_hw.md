...
## 1          0
```
The amniota data set does not have any NA values, which does not make sense. The NA values are represented by -999
...
```
These results make sense biologically. The "egg_mass_g" column has 100% missing values because mammals do not have eggs. In this case, this isn't missing data, but instead it is something that is just not applicable to mammals. 
...
## 4 Ter        1104     16.3
```
These rows are likely stating whether the different species do or do not classify as fossorial, terrestrial, aquatic, or arboreal. The only other value in these columns is 1, so 1 is likely being used to mean that a species does fall under one of those categories, whereas NA means the species does not. 
